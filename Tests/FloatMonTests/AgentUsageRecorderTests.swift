...
final class AgentUsageRecorderTests: XCTestCase {
    ...
    func testRecordsOnlyPositiveTokenDeltasAfterBaseline() throws {
        ...
        let recorder = AgentUsageRecorder(paths: testPaths, now: { now })

        try recorder.record(samples: [
            AgentThreadTokenSample(id: "thread-1", tokensUsed: 100, updatedAtMS: milliseconds(for: now)),
            AgentThreadTokenSample(id: "thread-2", tokensUsed: 50, updatedAtMS: milliseconds(for: now))
        ])
        ...
        try recorder.record(samples: [
            AgentThreadTokenSample(id: "thread-1", tokensUsed: 175, updatedAtMS: milliseconds(for: now)),
            AgentThreadTokenSample(id: "thread-2", tokensUsed: 40, updatedAtMS: milliseconds(for: now)),
            AgentThreadTokenSample(id: "thread-3", tokensUsed: 200, updatedAtMS: milliseconds(for: now))
        ])
    ...
    func testRecordsDeltasIntoCurrentCaptureDay() throws {
        ...
        let recorder = AgentUsageRecorder(paths: testPaths, now: { currentDate })

        try recorder.record(samples: [
            AgentThreadTokenSample(id: "thread-1", tokensUsed: 100, updatedAtMS: milliseconds(for: firstDay))
        ])
        try recorder.record(samples: [
            AgentThreadTokenSample(id: "thread-1", tokensUsed: 125, updatedAtMS: milliseconds(for: firstDay))
        ])
        currentDate = secondDay
        try recorder.record(samples: [
            AgentThreadTokenSample(id: "thread-1", tokensUsed: 175, updatedAtMS: milliseconds(for: secondDay))
        ])
    ...
    func testProviderRecorderWritesToProviderSpecificSQLite() throws {
        let calendar = Calendar(identifier: .gregorian)
        let now = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 29, hour: 12)))
        let recorder = AgentUsageRecorder(paths: testPaths, provider: .opencode, now: { now })

        try recorder.record(samples: [
            AgentThreadTokenSample(id: "session-1", tokensUsed: 100, updatedAtMS: milliseconds(for: now))
        ])

        XCTAssertTrue(FileManager.default.fileExists(atPath: testPaths.usageSQLite(provider: .opencode).path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: testPaths.usageSQLite(provider: .codex).path))
    }
